...
for i in index_possible_area:
    ...
    prod_scal_patch = abs(sum(prod)) #QUESTO MI DICE QUANTO LA PATCH E' PIATTA
    #if prod_scal_patch != 0.0:
    #    print(abs(math.log10(prod_scal_patch)))
    #if prod_scal_patch > Max_scalar: # SALVO GLI INDICI DEI PUNTI DOVE SERVE CAMPIONAMENTO PIU' FITTO
    if prod_scal_patch != 0.0:
        ...
        for k in range(len(patch)):
            ...
            #print("d[{}]=".format(k), d2[k])
            #print("value", abs((Rs_select / divided) * math.log10(prod_scal_patch)))
            if d2[k] < abs((Rs_select/divided)*math.log10(prod_scal_patch)): #SALVO I PUNTI NEL CENTRO
        ...
        #if len(patch) != len(center_index) & len(center_index) != 0:
        #    print("points in the patch", len(patch))
        #    print("points to be discarded", len(center_index))
print("tot=", len(index_possible_area))
