    ...
    while(Nleft > 0):
        ...
        while(count > 0):
            ...
            for i in pos_s[0]:
                ...
                #sys.stderr.write("\rPoints left: %d | N = %d"%(Nleft,N))
                #sys.stderr.flush()
...
def ContactPoints_NewVersion(list_1, list_2, thresh, PRINT_PROGRESS=True):
    '''
    This function finds the groups of point of list1 and list2 that have a distance lesser that thresh 
    from at least one point of the other list.
    '''
    ...
    for i in range(L1):
        if(i%1000 == 0 and PRINT_PROGRESS):
            sys.stderr.write("\rProgress %d / %d"%(i, L1))
        d2 = (list_1[i,0] - list_2[:,0])**2 + (list_1[i,1] - list_2[:,1])**2 + (list_1[i,2] - list_2[:,2])**2
        mask = d2 < thresh2
...
class Surface:
    ...
    def EnlargePixels(self, plane, THRES = 400):
    ...
    def BuildPatchNoIS(self, point_pos, Dmin):
    ...
    def CreateVoxelization(self, surface, Np, SHAPE = True, ELECT = False):
        surf_test =  surface[:,:3].copy()       
        cm = np.mean(surf_test[:,:3], axis=0)
        
        # shifting patch to have the cone origin in [0,0,0]...
        surf_test[:,:] -= cm
        
        #computing distances between points and the origin..
        weigths = np.sqrt(surf_test[:,0]**2 +  surf_test[:,1]**2 + surf_test[:,2]**2)
        
        R = np.max(weigths)*1.2
        
        surf_test += np.array([R,R,R])
        
        if(SHAPE):
            mymat_sh = np.zeros((Np,Np,Np))
        if(ELECT):
            mymat_el = np.zeros((Np,Np,Np))

        surf_test *= Np/(2*R)
            
        my_pos, inv = np.unique(surf_test.astype(int), axis=0, return_inverse=True)
    
        if(SHAPE):
            mymat_sh[my_pos[:,0].astype(int),my_pos[:,1].astype(int),my_pos[:,2].astype(int)] = 1
    
        if(ELECT):
            count =0
            for i in range(len(my_pos[:,0])):
                mask = inv == count
                el_val = np.mean(data[mask,3])
                #print(el_val)
                mymat_el[my_pos[i,0], my_pos[i,1], my_pos[i,2]] = el_val
                count += 1
    
        if(ELECT and SHAPE):
            return(mymat_sh, mymat_el)
        elif(ELECT and not SHAPE):
            return(mymat_el)
        elif(SHAPE and not ELECT):
            return(mymat_sh)
        else:
            print("Error! Select at least one between SHAPE and ELECT!")
            return(-1)
    ...
    def CreatePlane(self, patch, z_c,Np = 20, SHAPE = True, ELECT = False):
        ...
        if(SHAPE):
            plane_sh = np.zeros((Np,Np))
        if(ELECT):
            plane_el = np.zeros((Np,Np))
        ...
        if(SHAPE and ELECT):
            return(plane_sh, plane_el, weigths, dist_plane, thetas)
        elif(SHAPE):
            return(plane_sh, weigths, dist_plane, thetas)
    ...
    def FindOrigin(self,rotated_pacth, CHECK = 0):
        ...
        while(np.sum(mask) != 0 and ccc < 1000):
